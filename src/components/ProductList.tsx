...
const ProductList = async ({
  ...
  var productQuery = wixClient.products
    .queryProducts()
  ...
  var res = await productQuery.find();
    ...
    <div className="mt-12 flex gap-x-8 gap-y-16 justify-between flex-wrap">
      {res.items.map((product: products.Product) => (
        ...
        >
          ...
          <button className="rounded-2xl ring-1 ring-lama text-lama w-max py-2 px-4 text-xs hover:text-white hover:bg-lama">
            Add to Cart
          </button>
        </Link>
      ))}
